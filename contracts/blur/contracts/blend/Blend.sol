...
contract Blend is IBlend, OfferController, UUPSUpgradeable {
    ...
    function borrow(
        ...
        offer.collection.safeTransferFrom(
            msg.sender, address(this), collateralTokenId
        );
        ...
        lienIsActive(lien)
    {
        ...
        lien.collection.safeTransferFrom(
            address(this), lien.borrower, lien.tokenId
        );
     ...
     */
    function _repay(Lien calldata lien, uint256 lienId)
        internal
        returns (uint256 debt)
    {
        debt =
            Helpers.computeCurrentDebt(lien.amount, lien.rate, lien.startTime);
     ...
     */
    function startAuction(Lien calldata lien, uint256 lienId)
        external
        validateLien(lien, lienId)
    {
    ...
    function seize(LienPointer[] calldata lienPointers) external {
        ...
        for (uint256 i; i < length;) {
            ...
            if (_lienIsDefaulted(lien)) {
                ...
                lien.collection.safeTransferFrom(
                    address(this), lien.lender, lien.tokenId
                );
    ...
    function borrowerRefinance(
        ...
        if (loanAmount >= debt) {
            ...
            unchecked {
                _POOL.transferFrom(
                    offer.lender, lien.borrower, loanAmount - debt
                );
            }
        } else {
            ...
            unchecked {
                _POOL.transferFrom(
                    lien.borrower, lien.lender, debt - loanAmount
                );
            }
        }
    }
        ...
        returns (uint256 lienId)
    {
        ...
        (uint256 priceAfterFees, uint256 debt) =
            _buyLocked(lien, sellInput.offer, sellInput.signature);

        lienId = _borrow(
            loanInput.offer, loanInput.signature, loanAmount, lien.tokenId
        );
        ...
        if (loanAmount < debt) {
            ...
            unchecked {
                ...
                _POOL.transferFrom(
                    msg.sender, sellInput.offer.borrower, priceAfterFees - debt
                );
            }
    ...
    function buyLocked(
        ...
        (uint256 priceAfterFees, uint256 debt) =
            _buyLocked(lien, offer, signature);
    ...
    function takeBidV2(
        ...
        Helpers.executeTakeBidV2(
            lien, execution, debt, _POOL, _EXCHANGE_V2, _DELEGATE_V2
        );
    }
    ...
    function _takeSellOffer(
        ...
        /* Transfer fees. */
        uint256 totalFees =
            _transferFees(sellOffer.fees, msg.sender, sellOffer.price);
        unchecked {
    ...
    function _validateLien(Lien calldata lien, uint256 lienId)
        internal
        view
        returns (bool)
    {
    ...
    function _lienIsDefaulted(Lien calldata lien)
        internal
        view
        returns (bool)
    {
        return lien.auctionStartBlock != 0
            && lien.auctionStartBlock + lien.auctionDuration < block.number;
    }

    function _auctionIsActive(Lien calldata lien)
        internal
        view
        returns (bool)
    {
